//
//  YHorizontalTableViewDelegate.h
//  HorizontalTableView
//
...
@class YHorizontalTableView;
@class YHorizontalCell;


@protocol YHorizontalTableViewDelegate <NSObject>
@optional

// Display customization

- (void)tableView:(nonnull YHorizontalTableView *)tableView willDisplayCell:(nonnull YHorizontalCell *)cell forRowAtIndexPath:(nonnull NSIndexPath *)indexPath;
- (void)tableView:(nullable YHorizontalTableView *)tableView willDisplayHeaderView:(nullable UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0);
- (void)tableView:(nullable  YHorizontalTableView *)tableView willDisplayFooterView:(nullable UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0);
- (void)tableView:(nullable YHorizontalTableView *)tableView didEndDisplayingCell:(nullable YHorizontalCell *)cell forRowAtIndexPath:(nullable NSIndexPath*)indexPath NS_AVAILABLE_IOS(6_0);
- (void)tableView:(nullable YHorizontalTableView *)tableView didEndDisplayingHeaderView:(nullable UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0);
- (void)tableView:(nullable YHorizontalTableView *)tableView didEndDisplayingFooterView:(nullable UIView *)view forSection:(NSInteger)section NS_AVAILABLE_IOS(6_0);

// Variable height support

- (CGFloat)h_tableView:(nullable YHorizontalTableView *)tableView heightForRowAtIndexPath:(nullable NSIndexPath *)indexPath;
- (CGFloat)tableView:(nullable YHorizontalTableView *)tableView heightForHeaderInSection:(NSInteger)section;
- (CGFloat)tableView:(nullable YHorizontalTableView *)tableView heightForFooterInSection:(NSInteger)section;
/*
...
@protocol YHorizontalTableViewDataSource <NSObject>

@required

- (NSInteger)h_tableView:(nonnull YHorizontalTableView *)tableView numberOfRowsInSection:(NSInteger)section;

// Row display. Implementers should *always* try to reuse cells by setting each cell's reuseIdentifier and querying for available reusable cells with dequeueReusableCellWithIdentifier:
// Cell gets various attributes set automatically based on table (separators) and data source (accessory views, editing controls)

- (nullable YHorizontalCell *)h_tableView:(nonnull YHorizontalTableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath;

@optional

- (NSInteger)numberOfSectionsInTableView:(nullable YHorizontalTableView *)tableView;              // Default is 1 if not implemented

- (nullable NSString *)tableView:(nullable YHorizontalTableView *)tableView titleForHeaderInSection:(NSInteger)section;    // fixed font style. use custom view (UILabel) if you want something different
- (nullable NSString *)tableView:(nullable YHorizontalTableView *)tableView titleForFooterInSection:(NSInteger)section;

// Editing

// Individual rows can opt out of having the -editing property set for them. If not implemented, all rows are assumed to be editable.
- (BOOL)tableView:(nullable YHorizontalTableView *)tableView canEditRowAtIndexPath:(nullable NSIndexPath *)indexPath;

// Moving/reordering

// Allows the reorder accessory view to optionally be shown for a particular row. By default, the reorder control will be shown only if the datasource implements -tableView:moveRowAtIndexPath:toIndexPath:
- (BOOL)tableView:(nullable YHorizontalTableView *)tableView canMoveRowAtIndexPath:(nullable NSIndexPath *)indexPath;

// Index

- (nullable NSArray<NSString *> *)sectionIndexTitlesForTableView:(nullable YHorizontalTableView *)tableView __TVOS_PROHIBITED;                                                    // return list of section titles to display in section index view (e.g. "ABCD...Z#")
- (NSInteger)tableView:(nullable YHorizontalTableView *)tableView sectionForSectionIndexTitle:(nullable NSString *)title atIndex:(NSInteger)index __TVOS_PROHIBITED;  // tell table which section corresponds to section title/index (e.g. "B",1))

// Data manipulation - insert and delete support

// After a row has the minus or plus button invoked (based on the UITableViewCellEditingStyle for the cell), the dataSource must commit the change
// Not called for edit actions using UITableViewRowAction - the action's handler will be invoked instead
- (void)tableView:(nullable YHorizontalTableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(nullable NSIndexPath *)indexPath;

// Data manipulation - reorder / moving support

- (void)tableView:(nullable YHorizontalTableView *)tableView moveRowAtIndexPath:(nullable NSIndexPath *)sourceIndexPath toIndexPath:(nullable NSIndexPath *)destinationIndexPath;

@end



@interface YHorizontalTableViewDelegate : NSObject
